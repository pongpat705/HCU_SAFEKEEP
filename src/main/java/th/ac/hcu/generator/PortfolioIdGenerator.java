...
public class PortfolioIdGenerator implements IdentifierGenerator{
	...
	public Serializable generate(SessionImplementor session, Object object) throws HibernateException {
		...
		try {
			...
			PreparedStatement ps = connection .prepareStatement("SELECT sequence_next_hi_value as value from hibernate_sequences where upper(sequence_name) = upper('master_portfolio') ");
			...
			if (rs.next()) {
	        	...
	        	sb.append(" UPDATE hibernate_sequences SET sequence_next_hi_value = '"+id+"'  WHERE upper(sequence_name) = upper('master_portfolio') ");
	        	PreparedStatement update = connection .prepareStatement(sb.toString());
	        	update.executeUpdate();
	        } else {
	        	StringBuilder sb = new StringBuilder();
	        	sb.append(" INSERT INTO hibernate_sequences (sequence_name, sequence_next_hi_value) VALUES('master_portfolio', '1') ");
